...
class Vehicle(object):
    ...
    def refresh_token(self):
        _LOGGER.debug(f"{DOMAIN} - Refresh token started {self.token.valid_until} {datetime.now()}")
    ...
    async def set_last_updated(self):
        m = re.match(r"(\d{4})(\d{2})(\d{2})(\d{2})(\d{2})(\d{2})", self.vehicle_data["vehicleStatus"]["time"])
